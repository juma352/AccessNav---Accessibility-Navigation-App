...
import { LocalResource as LocalResourceType } from '../../types';
import { MapPin, Star, Phone, Accessibility } from 'lucide-react';

interface LocalResourceProps {
  resource: LocalResourceType;
  onGetDirections: (resourceId: string) => void;
  onCall: (resourceId: string) => void;
  onViewDetails: (resourceId: string) => void;
  onReview: (resourceId: string) => void;
  distance?: string;
}

export const LocalResource: React.FC<LocalResourceProps> = ({ 
  resource, 
  onGetDirections, 
  onCall, 
  onViewDetails,
    ...
    <AccessibleCard className="hover:shadow-xl transition-shadow">
      <div className="space-y-4">
        ...
        <div className="flex gap-3 pt-4">
          <AccessibleButton
            onClick={() => onGetDirections(resource.id)}
            variant="primary"
            size="md"
